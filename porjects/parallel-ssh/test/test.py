...
class PsshTest(unittest.TestCase):
    ...
    def testShortOpts(self):
        ...
        hostsFile.flush()
        cmd = "%s/bin/pssh -h %s -l %s -p 64 -o %s -e %s -t 60 -v -P -i uptime < /dev/null" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        rv = os.system(cmd)
    ...
    def testLongOpts(self):
        ...
        hostsFile.write("".join(map(lambda x: "%s\n" % x, g_hosts)))
        hostsFile.flush()
        cmd = "%s/bin/pssh --hosts=%s --user=%s --par=64 --outdir=%s --errdir=%s --timeout=60 --verbose --print --inline uptime < /dev/null" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        rv = os.system(cmd)
        self.assertEqual(rv, 0)
        for host in g_hosts:
    ...
    def testStderr(self):
        ...
        hostsFile.write("".join(map(lambda x: "%s\n" % x, g_hosts)))
        hostsFile.flush()
        cmd = "%s/bin/pssh -h %s -l %s -p 64 -o %s -e %s -t 60 -v -P -i ls /foobarbaz < /dev/null" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        rv = os.system(cmd)
        self.assertEqual(rv, 0)
        for host in g_hosts:
...
class PscpTest(unittest.TestCase):
    ...
    def testShortOpts(self):
        ...
        hostsFile.write("".join(map(lambda x: "%s\n" % x, g_hosts)))
        hostsFile.flush()
        cmd = "%s/bin/pscp -h %s -l %s -p 64 -o %s -e %s -t 60 /etc/hosts /tmp/pssh.test < /dev/null" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        rv = os.system(cmd)
        self.assertEqual(rv, 0)
        for host in g_hosts:
    ...
    def testLongOpts(self):
        ...
        hostsFile.write("".join(map(lambda x: "%s\n" % x, g_hosts)))
        hostsFile.flush()
        cmd = "%s/bin/pscp --hosts=%s --user=%s --par=64 --outdir=%s --errdir=%s --timeout=60 /etc/hosts /tmp/pssh.test < /dev/null" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        rv = os.system(cmd)
        self.assertEqual(rv, 0)
        for host in g_hosts:
    ...
    def testRecursive(self):
        ...
        hostsFile.write("".join(map(lambda x: "%s\n" % x, g_hosts)))
        hostsFile.flush()
        cmd = "%s/bin/pscp -r -h %s -l %s -p 64 -o %s -e %s -t 60 /etc/init.d /tmp/pssh.test < /dev/null" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        rv = os.system(cmd)
        self.assertEqual(rv, 0)
...
class PslurpTest(unittest.TestCase):
    ...
    def testShortOpts(self):
        ...
        hostsFile.write("".join(map(lambda x: "%s\n" % x, g_hosts)))
        hostsFile.flush()
        cmd = "%s/bin/pslurp -L /tmp/pssh.test -h %s -l %s -p 64 -o %s -e %s -t 60 /etc/hosts hosts < /dev/null" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        rv = os.system(cmd)
        self.assertEqual(rv, 0)
    ...
    def testLongOpts(self):
        ...
        hostsFile.write("".join(map(lambda x: "%s\n" % x, g_hosts)))
        hostsFile.flush()
        cmd = "%s/bin/pslurp --localdir=/tmp/pssh.test --hosts=%s --user=%s --par=64 --outdir=%s --errdir=%s --timeout=60 /etc/hosts hosts < /dev/null" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        rv = os.system(cmd)
        self.assertEqual(rv, 0)
    ...
    def testRecursive(self):
        ...
        hostsFile.write("".join(map(lambda x: "%s\n" % x, g_hosts)))
        hostsFile.flush()
        cmd = "%s/bin/pslurp -r -L /tmp/pssh.test -h %s -l %s -p 64 -o %s -e %s -t 60 /etc/init.d init.d < /dev/null" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        rv = os.system(cmd)
        self.assertEqual(rv, 0)
        ...
        for host in g_hosts:
            ...
            data = os.popen(cmd).read()
            self.assertEqual(data, os.popen(
                "ls -R /tmp/pssh.test/%s/init.d | sed 1d | sort" % host).read())
...
class PrsyncTest(unittest.TestCase):
    ...
    def testShortOpts(self):
        ...
        hostsFile.write("".join(map(lambda x: "%s\n" % x, g_hosts)))
        hostsFile.flush()
        cmd = "%s/bin/prsync -h %s -l %s -p 64 -o %s -e %s -t 60 -a -z /etc/hosts /tmp/pssh.test < /dev/null" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        rv = os.system(cmd)
        self.assertEqual(rv, 0)
        for host in g_hosts:
    ...
    def testLongOpts(self):
        ...
        hostsFile.write("".join(map(lambda x: "%s\n" % x, g_hosts)))
        hostsFile.flush()
        cmd = "%s/bin/prsync --hosts=%s --user=%s --par=64 --outdir=%s --errdir=%s --timeout=60 --archive --compress /etc/hosts /tmp/pssh.test < /dev/null" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        rv = os.system(cmd)
        self.assertEqual(rv, 0)
        for host in g_hosts:
    ...
    def testRecursive(self):
        ...
        hostsFile.write("".join(map(lambda x: "%s\n" % x, g_hosts)))
        hostsFile.flush()
        cmd = "%s/bin/prsync -r -h %s -l %s -p 64 -o %s -e %s -t 60 -a -z /etc/init.d/ /tmp/pssh.test < /dev/null" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        rv = os.system(cmd)
        self.assertEqual(rv, 0)
        files = os.popen("ls -R /etc/init.d | sed 1d | sort").read().strip()
...
class PnukeTest(unittest.TestCase):
    ...
    def testShortOpts(self):
        ...
        hostsFile.write("".join(map(lambda x: "%s\n" % x, g_hosts)))
        hostsFile.flush()
        cmd = "%s/bin/pssh -h %s -l %s -p 64 -o %s -e %s -t 60 -v sleep 60 < /dev/null &" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        os.system(cmd)
        time.sleep(5)

        cmd = "%s/bin/pnuke -h %s -l %s -p 64 -o %s -e %s -t 60 -v sleep < /dev/null" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        print cmd
        rv = os.system(cmd)
        self.assertEqual(rv, 0)
    ...
    def testLongOpts(self):
        ...
        hostsFile.write("".join(map(lambda x: "%s\n" % x, g_hosts)))
        hostsFile.flush()
        cmd = "%s/bin/pssh --hosts=%s --user=%s --par=64 --outdir=%s --errdir=%s --timeout=60 --verbose sleep 60 < /dev/null &" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        os.system(cmd)
        time.sleep(5)

        cmd = "%s/bin/pnuke --hosts=%s --user=%s --par=64 --outdir=%s --errdir=%s --timeout=60 --verbose sleep < /dev/null" % (
            basedir, hostsFile.name, g_user, self.outDir, self.errDir)
        print cmd
        rv = os.system(cmd)
        self.assertEqual(rv, 0)
