...
start_recovery() {
    for i in $(find "${PGDATA}"/pg_wal -maxdepth 1 -type f -regex ".*?/[0-9A-F]+"); do
        test -f "${PGDATA}"/../backup/wal/$(basename "${i}") || gosu postgres zstd -f -q -o "${PGDATA}"/../backup/wal/$(basename "${i}") "${i}"
    done
    rm -rf "${PGDATA}"/*
    ...
    gosu postgres cp /usr/share/postgresql/10/recovery.conf.sample "${PGDATA}"/recovery.conf
    ...
    if start_pg; then
        ...
        gosu postgres tar -C "${PGDATA}" -zcpf "${PGDATA}"/../backup/base_database.tar.gz ./
